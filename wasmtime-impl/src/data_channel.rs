...
/// SCTP send-buffer bound the jco hosts use.
...
impl InboundBudget {
    ...
    /// if the reservation would exceed [`MAX_INBOUND_BUFFER_BYTES`] or an
    /// overflow was already latched.
    pub fn reserve(&self, len: usize) -> bool {
        if self.overflowed.load(Ordering::SeqCst) {
            return false;
        }
        if self.buffered.load(Ordering::SeqCst).saturating_add(len) > MAX_INBOUND_BUFFER_BYTES {
